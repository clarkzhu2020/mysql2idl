...
func newMigration(db *gorm.DB, opts ...gen.DOOption) migration {
	...
	_migration.migrationDo.UseModel(&tables.Migration{})

	tableName := _migration.migrationDo.TableName()
	_migration.ALL = field.NewAsterisk(tableName)
	_migration.Version = field.NewInt64(tableName, "version")
	_migration.Dirty = field.NewBool(tableName, "dirty")
...
func (m *migration) updateTableName(table string) *migration {
	...
	m.Dirty = field.NewBool(table, "dirty")
...
func (m migrationDo) Create(values ...*tables.Migration) error {
	if len(values) == 0 {
		return nil
	}
...
func (m migrationDo) CreateInBatches(values []*tables.Migration, batchSize int) error {
	return m.DO.CreateInBatches(values, batchSize)
}

// Save : !!! underlying implementation is different with GORM
// The method is equivalent to executing the statement: db.Clauses(clause.OnConflict{UpdateAll: true}).Create(values)
func (m migrationDo) Save(values ...*tables.Migration) error {
	if len(values) == 0 {
		return nil
	}
...
func (m migrationDo) First() (*tables.Migration, error) {
	if result, err := m.DO.First(); err != nil {
		return nil, err
	} else {
		return result.(*tables.Migration), nil
	}
}

func (m migrationDo) Take() (*tables.Migration, error) {
...
func (m migrationDo) Last() (*tables.Migration, error) {
...
func (m migrationDo) Find() ([]*tables.Migration, error) {
	result, err := m.DO.Find()
	return result.([]*tables.Migration), err
}

func (m migrationDo) FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*tables.Migration, err error) {
	buf := make([]*tables.Migration, 0, batchSize)
...
func (m migrationDo) FindInBatches(result *[]*tables.Migration, batchSize int, fc func(tx gen.Dao, batch int) error) error {
...
func (m migrationDo) FirstOrInit() (*tables.Migration, error) {
...
func (m migrationDo) FirstOrCreate() (*tables.Migration, error) {
...
func (m migrationDo) FindByPage(offset int, limit int) (result []*tables.Migration, count int64, err error) {
...
func (m migrationDo) Delete(models ...*tables.Migration) (result gen.ResultInfo, err error) {
